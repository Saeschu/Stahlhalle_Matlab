...
for z1=[1:length(F)]
if x(z1)<=(b/2)
    Va40=(1/12).*((3-4.*((x(z1)./b).^2))/(1-(x(z1)./b))).*(-0.25.*b).*(Av0(z1).*x(z1)).*b
elseif x(z1)>(b/2)                                      %0 mit 4 Zustand 
    Va40=(1/12).*((3-4.*((1-(x(z1)./b)).^2))/(x(z1)./b)).*(-0.25.*b).*(Av0(z1).*x(z1)).*b
end
end
...
WerteSTR=linspace(sum(Mb),sum(Mr),dx)                   %Superposition Stab Rechts Werte Moment
 ...
 for z1=[1:length(F)]
    ...
    elseif x(z1)<0.5*b            
    xSTOLV=linspace(Ml(z1),Mf(z1),(x(z1).*dx))                  %Moment Stab oben links von F
    xSTOMV=linspace(Mf(z1),Ms1(z1),((0.5*b-(x(z1))).*dx))
    xSTORV=linspace(Ms1(z1),Mr(z1),((0.5*b).*dx))               %Moment Stab oben rechts von F
    VorWerteSTO{z1}=[xSTOLV(1:end-1),xSTOMV,xSTORV(2:end)]    %Moment Stab oben links + rechts
    elseif x(z1)==b*0.5 
    VorWerteSTO{z1}=zeros(1,((b*dx)-2))                     %Momente Stab oben wenn Moment = 0 
    elseif x(z1)==b 
    VorWerteSTO{z1}=zeros(1,((b*dx)-2))
    elseif x(z1)>0.5*b            
    xSTOLV=linspace(Ml(z1),Ms1(z1),((0.5*b).*dx))        %Moment Stab oben links von F
    xSTOMV=linspace(Ms1(z1),Mf(z1),((x(z1)-(0.5*b)).*dx))
    xSTORV=linspace(Mf(z1),Mr(z1),((b-(x(z1))).*dx))  %Moment Stab oben rechts von F
    VorWerteSTO{z1}=[xSTOLV(1:end-1),xSTOMV,xSTORV(2:end)]    %Moment Stab oben links + rechts
